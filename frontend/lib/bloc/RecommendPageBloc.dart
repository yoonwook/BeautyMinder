...
class RecommendPageBloc extends Bloc<RecommendPageEvent, RecommendState> {
  ...
  Future<void> _initEvent(
      RecommendPageInitEvent event, Emitter<RecommendState> emit) async {
  ...
  Future<void> _categoryChangeEvent(RecommendPageCategoryChangeEvent event,
      Emitter<RecommendState> emit) async {
    await Future.delayed(const Duration(seconds: 0), () async {
      ...
      if (state is RecommendLoadedState) {
        ...
        if (event.category == null) {
          ...
          emit(RecommendCategoryChangeState(
              category: state.category,
              isError: state.isError,
              recCosmetics: AllCosmetics));
        } else {
